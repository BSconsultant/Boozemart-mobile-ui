...
const [myorderlist, setMyorderlist] = useState([]);
    ...
    useEffect(() => {
        if(!props.route.params){
            Axios.get(ApiUrls.SERVICE_URL+"/order/all", {
                                  headers: {
                                  userId: 1
                                 }}).then((succResp) =>{
        ...
        Axios.get(ApiUrls.SERVICE_URL+"/order/ByStatusAndUserId/"+props.route.params.orderType, {
          headers: {
            user_id: 1
          }}).then((succResp) =>{
              setMyorderlist(succResp.data.map((item) => {
